...
- [x] **제출 패키지 동결(frozen) 정책 명시** — `6eac233`
  ...
  - 왜: 제출 패키지의 "50개 테스트" 로그가 stale가 아니라 동결본임을 분명히 함(현재 HEAD의 114개와 혼동 방지).
  - 검증: README 내용 확인.
...
- [x] **멀티턴 대화에 assistant 메시지 이력 누적** — 이후 전체 기능 리뷰에서 실제 결함으로 재분류되어 test-first로 적용. assistant 응답을 히스토리에 남기고 다음 요청 컨텍스트에 포함함.
...
- [x] `npm test` 114개 전부 통과(테스트 삭제/스킵 없음)
